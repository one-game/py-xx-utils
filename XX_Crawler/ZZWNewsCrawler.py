...
import re
from XXNewsCrawler import NewsCrawler
import logging
import json
import time
...
class ZZWNewsCrawler(NewsCrawler):
    ...
    def __init__(self, list_url_tpl, data_path):
        NewsCrawler.__init__(self, list_url_tpl, data_path)
    ...
    def parse_news_page(self, page_content):
        '''parse news page
        ...
        news_meta_pattern = re.compile(u'<span><em>(.*?)：<\/em>(.*?)<\/span>')
...
if __name__ == "__main__":
    url_tpl = "http://www.cs.com.cn/ssgs/"
    crawler = ZZWNewsCrawler(url_tpl, "./")
    crawler.run()
